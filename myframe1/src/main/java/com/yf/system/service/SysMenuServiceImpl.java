...
import com.yf.system.entity.MenuTree;
import com.yf.system.entity.SysMenu;
import com.yf.system.mapper.SysMenuMapper;
import com.yf.system.mapper.SysRoleMenuMapper;
import com.yf.util.JSONUtils;
import com.yf.util.TreeUtil;
...
public class SysMenuServiceImpl extends BaseServiceImpl<SysMenu, Long>
	...
	public int removeById(Long id) {
		...
		int flag = mapper.removeByCondition(condition);
		return flag;
	}
		...
		if(list!=null && list.size()>0){
			for(SysMenu menu : list){
				if(menu.getMenuId()==menuId){ //删除当前菜单
					...
					roleMenuMapper.removeByCondition(relativeCondition);
				}
